...
const getDataItemFilteredEntries = (item, columns) => {
  ...
  return columnsKeys.length > 0 ? itemEntries.filter(([key]) => columnsKeys.includes(key)) : itemEntries;
};
...
const getComputedData = (data, columns, mutations) => {
  ...
  for (const item of data) {
    ...
    filteredEntries.map(([key, val]) => (computedItem[key] = applyMutations(key, val, mutations)));
    computedData.push(computedItem);
  }
  return computedData;
};

const haldleValue = (value) => (Validators.isTimeString(value) ? Mutations.TimeStringToSeconds(value) : 0);
...
const renderCells = (rowId, colsWidth, item, columns) =>
  Object.entries(item).map(([key, value]) => {
    ...
    const content = isObjectValue ? <span>{value || getEmptyPlaceholder()}</span> : getHtmlText(value);
    return (
      <td key={`${rowId}-td-${key}`} data-header={header} title={title} style={{ width: colsWidth }}>
        {content}
      </td>
    );
  });
  ...
  return (
    <td key={cellId} className="actions">
      {actions.map((a) => {
        ...
        return (
          <button key={`${cellId}-${icon}`} className="action" title={name} onClick={() => handler(item)}>
            <Icon name={icon} />
          </button>
        );
      })}
...
const sumDataItemValue = (data, key, totalizer) => {
  ...
  const sumBase = totalizer === Mutations.TotalizerType.AVERAGE ? data.length : 1;
  const value = sum / sumBase;
  return Validators.isTimeString(dataPattern) ? Mutations.SecondsToTime(value) : value;
};

const handleTotalizerType = (totalizer) => (totalizer === Mutations.TotalizerType.SUM ? 'Somatório' : 'Média');
...
const handleTotalizers = (data, columns, mutations, totalizers, colsWidth) => {
  ...
  if (totalizersKeys.some((key) => columnsKeys.includes(key))) {
    return columnsKeys.map((key) => {
      ...
      const value = hasTotalizer ? sumDataItemValue(data, key, totalizers[key]) : '';
      const mutation = hasTotalizer && mutations[key] ? mutations[key].handler : (val) => val;
  ...
  return (
    <div id={tableId} className={`app-table ${clsNoResults} ${clsActions}`}>
      <table className="thead">
        <thead>
          <tr key="header">{renderHeadings(tableId, computedColumns, columnsWidth)}</tr>
        </thead>
      </table>
        ...
        <table className="tfoot">
          <tfoot>
            <tr
              ...
              data-length={`Total: ${data.length} registro${data.length > 1 ? 's' : ''}`}
            >
              {!hasTotalizers && <td>&nbsp;</td>}
              {hasTotalizers && handleTotalizers(data, columns, mutations, totalizers, columnsWidth)}
            </tr>
          </tfoot>
        </table>
      )}
